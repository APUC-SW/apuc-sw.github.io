document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 중국어 파일 경로
    const markdownFilePath = `/global/wiki/doc/${docName}_zh.md`; // 경로는 올바름

    try {
        const response = await fetch(markdownFilePath);

        if (!response.ok) {
            console.warn(`Translation for ${docName} in 'zh' not found. Attempting to load original 'en' version.`);
